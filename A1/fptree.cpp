...
#define uint64_t int
using namespace std;
int MAX_ITER_DEPTH=12000;
struct CustomCompare {
    bool operator()(const std::pair<std::set<int>, int>& lhs, const std::pair<std::set<int>, int>& rhs) const {
        return (lhs.first.size()*lhs.second>rhs.first.size()*rhs.second);
    }
};
    ...
    minimum_support_threshold( minimum_support_threshold )
{
    ...
    std::unordered_map<Item, uint64_t> frequency_by_item;
...
std::vector<Pattern> fptree_growth(const FPTree& fptree, long double time_limit,std::chrono::system_clock::time_point &start_time )
{
    auto stop_timer = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::nanoseconds>(stop_timer - start_time);
    ...
    if ( contains_single_path( fptree ) ) {
        ...
        std::set<Pattern,CustomCompare> single_path_patterns;
        ...
        int index=0;
        for(auto &x:single_path_patterns){
            if(index>=1){
                break;
            }
            answer.push_back(x);
        ...
        std::set<Pattern,CustomCompare> multi_path_patterns;
        ...
        for ( const auto& pair : fptree.header_table ) {
            ...
            std::vector<Pattern> conditional_patterns = fptree_growth( conditional_fptree, time_limit,start_time);
        ...
        int index=0;
        for(auto &x:multi_path_patterns){
            if(index>=MAX_ITER_DEPTH){
                break;
            }
            answer.push_back(x);
            index++;
